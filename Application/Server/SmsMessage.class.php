...
class SmsMessage
{
    ...
    public function sendsubscribeSuccess($phone,$param)
    {
        ...
        if ($param['servicetime']) {
            ...
            $templateid = '2806';
            $tempparam[] = $tempparam['product'];
            $tempparam[] = $tempparam['servicetime'];
            $tempparam[] = $tempparam['code'];
     ...
     * 保险api监控短信预警
     */
    public function sendApiMonitorMsg($phone, $msg)
    {
        $msg = '【东家管家】Api Monitor'.$msg;
        vendor("ChuanglanSms.ChuanglanSmsApi");
        $smsApi = new \ChuanglanSmsApi();
        $res = $smsApi->sendSMS($phone,$msg);
