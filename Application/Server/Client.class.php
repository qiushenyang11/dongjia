...
class Client
{
	...
	public function close($code = self::CLOSE_NORMAL, $reason = '')
	{
		...
		return $this->socket->send(	\Swoole\WebSocket\Server::pack($data, self::OPCODE_CONNECTION_CLOSE, true));
	}
	...
	public function send($data, $type = 'text', $masked = false)
	{
		...
		return $this->socket->send(\Swoole\WebSocket\Server::pack($data, $_type, true, $masked));
	}
